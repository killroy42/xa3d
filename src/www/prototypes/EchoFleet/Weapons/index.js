	...
	createOnBeforeUpdateHandler() {
		...
		const handleOnBeforeUpdate = time => {
			...
			if(this.target !== undefined) {
				...
				targetV.subVectors(this.target, transform.getWorldPosition(position));
...
const shipAJson = {
	...
	Node: {children: [
		...
		{ // gun turret left
			Transform: {position: {x: 1, y: 0, z: 0}, rotation: {x: 0, y: 0, z: 0}},
			Node: {children: [turretJson]},
			WeaponController: {},
		},
		{ // gun turret right
			Transform: {position: {x: -1, y: 0, z: 0}, rotation: {x: 0, y: 0, z: 0}},
...
const initShips = entities => {
	...
	const configWeapon = ({weapon, a, v, target}) => {
		const weaponController = weapon.weaponController;
		weaponController.turnA = a;
		weaponController.turnV = v;
		weaponController.setTarget(target.position);
	};

	const ship = entities.createEntity(shipAJson);
	ship.transform.position.set(0, 0.2, 0);
	ship.shipBehaviour.setTarget(viewPos);
	
	const ship2 = entities.createEntity(shipBJson);
	ship2.transform.position.set(0, 1, 0);
	ship2.shipBehaviour.setTarget(movePos);


	configWeapon({
		weapon: ship.node.children[4].entity,
		a: 40 * Math.PI, v: 20 * Math.PI,
		target: ship2.transform
	});
	configWeapon({
		weapon: ship.node.children[5].entity,
		a: 40 * Math.PI, v: 20 * Math.PI,
		target: ship2.transform
	});

	configWeapon({
		weapon: ship2.node.children[4].entity,
		a: 0.1 * Math.PI, v: 0.5 * Math.PI,
		target: ship.transform
	});
	configWeapon({
		weapon: ship2.node.children[5].entity,
		a: 0.1 * Math.PI, v: 0.5 * Math.PI,
		target: ship.transform
	});
