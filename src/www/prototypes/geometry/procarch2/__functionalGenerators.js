...
const {
	...
	Shape, Geometry, ExtrudeGeometry,
	Object3D, Mesh, Line,
} = require('THREE');

const MAT4_MIRRORX = new Matrix4();
	MAT4_MIRRORX.elements[0] = -1;
	MAT4_MIRRORX.elements[10] = -1;
	MAT4_MIRRORX.elements[14] = -1;
const MAT4_MIRRORY = new Matrix4();
...
// Maker Functions
	...
	const makeOutlineExtrusion = ({outline, extrude}, generate) => new ExtrudeGeometry(new Shape(generate(outline)), extrude);
	...
	const makeWallMesh = ({seed, segments, width, height, depth, material}, generate) => {
		...
		const mesh = new Mesh(generate(geometry), material);
		mesh.name = 'wall';
	...
	const makeBulkheadMesh = ({seed, width, height, wall, ceiling, floor, bulkhead}, generate) => {
		...
		const shape = new Shape(generate(outline));
...
// Helpers
	const white = new LineBasicMaterial({color: 0xffffff});
	...
	const yellow = new LineBasicMaterial({color: 0xffff00});
...
const buildCornerSegment = ({seg0, seg1, wall, floor, ceiling}, generate) => {
	...
	// Floor
		var floorShape = new Shape([inside1, outside0, outside1, outside2]
			.map((v) => ({x: v.x - v1.x, y: v.z - v1.z})));
		var floorGeo = new ExtrudeGeometry(floorShape, {amount: floor.depth, bevelEnabled: false});
		floorGeo.rotateX(0.5 * Math.PI);
		//var ceilGeo = floorGeo.clone();
		//floorGeo.translate(0, -floor.depth, 0);
		const floorMesh = new Mesh(floorGeo, floor.material);
		corner.add(floorMesh);
	// Ceiling
		var ceilingGeo = new ExtrudeGeometry(floorShape, {amount: ceiling.depth, bevelEnabled: false});
		ceilingGeo.rotateX(0.5 * Math.PI);
		ceilingGeo.translate(0, wall0.scale.y, 0);
		ceilingGeo.translate(0, ceiling.depth, 0);
		const ceilingMesh = new Mesh(ceilingGeo, ceiling.material);
		corner.add(ceilingMesh);
	// Bulkhead
		const material = new MeshPhongMaterial({
			color: 0xffffff, shading: FlatShading
		});
		const opts = {
			seed: 0,
			width: 2,
...
module.exports.functionalGenerators = module.exports;
