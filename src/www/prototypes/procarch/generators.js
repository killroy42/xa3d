...
// Generator classes
	...
	class WallBuilder extends BuilderBase {
		generateGeometry({seed, segments, extrude}) {
			...
			const geometry = shape.extrude(extrude);
	...
	class CorridorAssembler extends AssemblerBase {
		...
		generate({wall, ceiling, floor, bulkhead}) {
			...
			const bulkheadShape = new THREE.Shape(this.getBulkheadOutline(arguments[0]));
			this.bulkheadGeometry = bulkheadShape.extrude(bulkhead.bevel);
